...
def main():
    ...
    # Load data - this returns pre-split training and validation sets
    df_train_base, df_train_update1, df_train_update2, df_val_base, df_val_update1, df_val_update2, df_test = load_and_prepare_data(data_dir)
    
    # Create combined datasets for scaling (needed for StandardScaler)
    df_base = pd.concat([df_train_base, df_val_base], ignore_index=True)
    df_update1 = pd.concat([df_train_update1, df_val_update1], ignore_index=True)
    df_update2 = pd.concat([df_train_update2, df_val_update2], ignore_index=True)
    ...
    # Extract scaled training and validation sets
    # Get indices of original dataframes
    train_base_idx = df_train_base.index
    val_base_idx = df_val_base.index
    train_update1_idx = df_train_update1.index
    val_update1_idx = df_val_update1.index
    train_update2_idx = df_train_update2.index
    val_update2_idx = df_val_update2.index
    
    # Re-split scaled data
    df_base_train = df_base_scaled.iloc[:len(df_train_base)].reset_index(drop=True)
    df_base_val = df_base_scaled.iloc[len(df_train_base):].reset_index(drop=True)
    df_update1_train = df_update1_scaled.iloc[:len(df_train_update1)].reset_index(drop=True)
    df_update1_val = df_update1_scaled.iloc[len(df_train_update1):].reset_index(drop=True)
    df_update2_train = df_update2_scaled.iloc[:len(df_train_update2)].reset_index(drop=True)
    df_update2_val = df_update2_scaled.iloc[len(df_train_update2):].reset_index(drop=True)
    ...
    # Split test set into 3 segments for each phase (base, update1, update2)
