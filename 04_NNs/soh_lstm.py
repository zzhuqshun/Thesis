...
save_dir = Path(__file__).parent / "models/LSTM" / datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
...
def main():
    ...
    plot_predictions(predictions, targets, metrics, df_test_scaled, results_dir)
...
def plot_predictions(predictions, targets, metrics, df_test_scaled, results_dir):
    """
    ...
    plt.plot(datetime_vals, targets, label='Actual SOH', lw=2)
    plt.plot(datetime_vals, predictions, label='Predicted SOH', alpha=0.7)
