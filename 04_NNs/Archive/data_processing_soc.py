...
def load_data(data_dir: str) -> pd.DataFrame:
    ...
    for file_path in tqdm(parquet_files, desc="Processing cells", unit="cell"):
        ...
        data = data[['Absolute_Time[yyyy-mm-dd hh:mm:ss]', 'Current[A]', 'Voltage[V]','Temperature[°C]', 'SOC_ZHU']]
        
        
        data_sampled = data.set_index('Absolute_Time[yyyy-mm-dd hh:mm:ss]').resample('s').mean()
        
        # fill missing values
        data_sampled.interpolate(method='linear', inplace=True)
        data_sampled = data_sampled.bfill()
        data_sampled.reset_index(drop=True, inplace=True)
        
        # add time_idx column
        data_sampled['Testtime[s]'] = data_sampled.index
        
        # add cell_id column
        data_sampled['cell_id'] = cell_name
        
        data_sampled = data_sampled[['Testtime[s]','Current[A]', 'Voltage[V]','Temperature[°C]', 'cell_id','SOC_ZHU']]
        
        df_list.append(data_sampled)
...
def plot_dataset_soh(data_df: pd.DataFrame, title: str, figsize=(10, 7)):
    ...
    for cell, group in data_df.groupby('cell_id'):
        plt.plot(group['Testtime[s]'], group['SOC_ZHU'], label=cell)
