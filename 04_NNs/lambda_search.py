...
TMP_ROOT = Path('lambda_search_incremental')
...
def prepare_base(config):
    ...
    state = torch.load(ckpt0, map_location=device, weights_only=True)
    torch.save({
        'model_state': state['model_state'],
        'ewc_tasks': [],  # Base 不做 EWC，留个空列表
...
def train_incremental(config, lambda1, lambda2, trial_id, base_ckpt_path):
    ...
    trainer.ewc_tasks = base_state['ewc_tasks']
