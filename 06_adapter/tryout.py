...
class Config:
    ...
    def __init__(self, **kwargs):
        ...
        # Directory structure
        self.BASE_DIR = Path.cwd() / "joint_gate"
...
class SOHLSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        avg_step = out.mean(dim=1)  # (batch, hidden) - 全序列平均
    
        # 自适应融合：学习如何在last_step和avg_step之间调节
        adjustment = torch.tanh(self.adaptive_fusion(last_step))  # (batch, 1)
        
        # 以avg_step为基础，根据adjustment调节
...
# ===============================================================
class Trainer:
    """Main training class for pure fine-tuning"""
    ...
    def train_task(self, train_loader, val_loader, task_id):
        """
        Train model on a single task using pure fine-tuning.
