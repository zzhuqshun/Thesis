...
class AdapterSOHLSTM(nn.Module):
    ...
    def __init__(self, input_size=3, hidden_size=128, dropout=0.3, 
        ...
        self.adapter = WindowedAttentionAdapter(
            hidden_size=hidden_size,
            window_size=window_size,
            overlap=overlap,
            reduction_factor=reduction_factor
    ...
    def forward(self, x):
        ...
        # LSTM + Adapter
        h2, _ = self.lstm(x)
        h2 = self.adapter(h2)  # Apply adapter after LSTM
    ...
    def get_adapter_params(self):
        ...
        adapter_params.extend(list(self.adapter.parameters()))
...
class CNNAdapter(nn.Module):
    """
    Convolutional adapter with bottleneck structure.
    Applies pointwise down-projection, 1D convolutions for temporal context,
    then up-projection, gated residual, and layer normalization.
    """
    def __init__(self, hidden_size=128, kernel_size=3, dilation=1, reduction_factor=8, dropout=0.1):
        super().__init__()
        self.hidden_size = hidden_size
    ...
    def forward(self, x):
        ...
        # down-project
        z = self.down_proj(x)               # [B, T, bottleneck]
        # prepare for conv: [B, bottleneck, T]
        z = z.transpose(1, 2)
        # conv→activation→dropout
        z = self.conv(z)
        z = self.activation(z)
        z = self.dropout(z)
        # back to [B, T, bottleneck]
        z = z.transpose(1, 2)
...
class AdapterCNN(nn.Module):
    """
    LSTM model with convolutional adapters for incremental learning.
    Adapters are placed after each LSTM layer to adapt representations.
    """
    def __init__(self, input_size=3, hidden_size=128, dropout=0.3, 
                 kernel_size=5, reduction_factor=8):
        super().__init__()
        
        # Base LSTM layers
        self.lstm1 = nn.LSTM(input_size, hidden_size, batch_first=True)
        self.adapter1 = CNNAdapter(
            hidden_size=hidden_size,
            kernel_size=kernel_size,
            dilation=1,
            reduction_factor=reduction_factor,
