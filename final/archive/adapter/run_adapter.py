...
from utils.adapter import AdapterSOHLSTM, AdapterTrainer
...
def adapter_learning(config):
    ...
    model = AdapterSOHLSTM(
        input_size=3, 
        hidden_size=config.HIDDEN_SIZE,
        dropout=config.DROPOUT,
        window_size=144,  # 24 hours with 10min sampling
        overlap=72,       # 50% overlap
    ...
    logger.info("  Window size: 144 (24 hours)")
    logger.info("  Overlap: 72 (50%%)")
    ...
    evaluate_incremental_learning(config, AdapterSOHLSTM, inc_dir, loaders, device)
...
def main():
    ...
    # Set up directories
    config.BASE_DIR = Path.cwd() / 'adapter_learning_lstm2layer'
