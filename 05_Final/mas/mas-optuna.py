...
TASK0_CONFIG = {
    ...
    "MAS_LAMBDAS": [0.0, 0.0, 0.0],  # 会被optuna覆盖
}
...
def objective(trial):
    ...
    # 采样MAS参数
    lam0 = trial.suggest_float("lambda0", 1e-1, 1e3, log=True)
    lam1 = trial.suggest_float("lambda1", 1e-1, 1e3, log=True)
    lam2 = 0.0  # 最后任务不需要MAS
    ...
    # 处理MAS任务数据
    if 'mas_tasks' not in state or len(state.get('mas_tasks', [])) == 0:
        ...
        # 保存更新的checkpoint
        serialized = []
        for reg in trainer.mas_tasks:
            serialized.append({
                'params': {n: p.cpu() for n, p in reg.params.items()},
                'importance': {n: imp.cpu() for n, imp in reg.importance.items()},
                'mas_lambda': reg.mas_lambda
            })
        torch.save({
            'model_state': trainer.model.state_dict(),
            'mas_tasks': serialized
        }, ckpt_path)
        print(f"Trial {trial.number}: Updated checkpoint with Task0 MAS info")
    else:
        trainer.model.load_state_dict(state['model_state'])
        # 重建MAS正则化器
        trainer.mas_tasks = []
        mas_task_data = state['mas_tasks']
        mas_reg = MAS.__new__(MAS)
        mas_reg.model = trainer.model
        mas_reg.device = DEVICE
        mas_reg.params = {n: p.to(DEVICE) for n, p in mas_task_data[0]['params'].items()}
        mas_reg.importance = {n: imp.to(DEVICE) for n, imp in mas_task_data[0]['importance'].items()}
        mas_reg.mas_lambda = lam0  # 使用当前试验的lambda
        trainer.mas_tasks.append(mas_reg)
    ...
    trainer.consolidate(cached_loaders['task1_train'], task_id=1, mas_lambda=lam1)
    ...
    trainer.consolidate(cached_loaders['task2_train'], task_id=2, mas_lambda=lam2)
...
def optimize_mas(n_trials=50):
    """执行MAS参数优化"""
    print(f"Starting MAS optimization with {n_trials} trials")
    ...
    study = optuna.create_study(
        ...
        study_name='mas_lambda_optimization'
    )
    ...
    print("\n" + "="*50)
    print("MAS OPTIMIZATION COMPLETED!")
    print("="*50)
