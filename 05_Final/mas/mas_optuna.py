...
def objective(trial):
    ...
    # 采样MAS参数
    lam0 = trial.suggest_float("lambda0", 1e-4, 1, log=True)
    lam1 = lam0
    lam2 = lam0  # 最后任务不需要MAS
    ...
    trainer.consolidate(cached_loaders['task0_train'], task_id=0, mas_lambda=lam0)

    # 取最后一个 epoch 的 mas_loss / task_loss
    ratio0 = history0['mas_loss'][-1] / (history0['task_loss'][-1] + 1e-12)
    ...
    ratio1 = history1['mas_loss'][-1] / (history1['task_loss'][-1] + 1e-12)
    ...
    ratio2 = history2['mas_loss'][-1] / (history2['task_loss'][-1] + 1e-12)
