...
def main(joint_training: bool = False):
    # config and logging
    config   = Config()
    base_dir = Path(__file__).parent / 'model' / 'windows_by_cell' 
...
def create_dataloaders(datasets, seq_len, batch_size):
    """ 生成 DataLoader 字典,按 cell_id 分组滑窗
     • datasets: dict[str, pd.DataFrame],键名如 'train', 'val', 'test_full' 等
     • seq_len: int,滑窗长度
     • batch_size: int,批大小

     返回:dict[str, DataLoader],键名与输入相同
     
     注意：
      • 旧的实现是直接对整个 DataFrame 滑窗,可能跨 cell_id 界限；
      • 这里改为先按 cell_id 分组,再滑窗,避免跨 cell 的问题。
    ...
    for key, df in datasets.items():
        if df.empty:
            continue
        if not any(tag in key for tag in ("train", "val", "test")):
            continue

        cell_dsets = [
            BatteryDataset(g, seq_len)
            for _, g in df.groupby("cell_id", sort=False)
        ]

        # 顺序拼接；因为我们不 shuffle,所以时间与 cell 顺序都保持
        if len(cell_dsets) == 1:
            final_ds = cell_dsets[0]
        else:
            final_ds = ConcatDataset(cell_dsets)

        loader = DataLoader(
            final_ds,
            batch_size=batch_size,
            shuffle=False,          # ⇦ 保证窗口时间顺序；如需随机可改 True
            num_workers=0,
        )
        loaders[key] = loader
    return loaders
