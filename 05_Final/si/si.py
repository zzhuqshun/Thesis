...
class SI:
    ...
    during training, then applies regularization to prevent important parameters
    from changing too much in subsequent tasks.
    ...
    def __init__(self, model, si_lambda=1.0, epsilon=0.1):
        ...
        self.prev_params = {n: p.data.clone().detach() 
                           for n, p in model.named_parameters() if p.requires_grad}
        
        # Importance weights (accumulated across tasks)
        self.omega = {n: torch.zeros_like(p.data) 
                     for n, p in model.named_parameters() if p.requires_grad}
        
        # Contribution accumulator for current task
    ...
    def begin_task(self):
        ...
        for name, param in self.model.named_parameters():
            if not param.requires_grad: 
                continue
            # Reset previous parameters to current values
            self.prev_params[name] = param.data.clone().detach()
            # Reset contribution accumulator
            self.w[name] = torch.zeros_like(param.data)
    
    def accumulate_contributions(self):
        """
        Accumulate parameter contributions during training.
        Called after each gradient computation but before parameter update.
        """
        for name, param in self.model.named_parameters():
            ...
            # Accumulate contribution: -gradient * parameter_change
            # This estimates how much each parameter contributes to loss reduction
    ...
    def end_task(self):
        """
        Update importance weights at the end of a task.
        Converts accumulated contributions to importance estimates.
        """
        for name, param in self.model.named_parameters():
            if not param.requires_grad: 
                continue
            
            # Final parameter change for this task
            delta = param.data - self.prev_params[name]
            
            # Update importance weights using accumulated contributions
            # omega += w / (delta^2 + epsilon)
            # Higher contribution + smaller change = higher importance
    ...
    def penalty(self):
        ...
        for name, param in self.model.named_parameters():
            ...
            # Calculate deviation from previous task parameters
            delta = param.data - self.prev_params[name]
            
            # Add weighted penalty: importance * parameter_change^2
            loss += torch.sum(self.omega[name] * delta**2)
    ...
    def train_task(self, train_loader, val_loader, task_id, alpha_lwf=0.0):
        ...
        for epoch in tqdm.tqdm(range(self.config.EPOCHS), desc=f"Task{task_id}"):
            ...
            for x, y in train_loader:
                ...
                # IMPORTANT: Accumulate SI contributions before parameter update
                if self.si is not None: 
                    self.si.accumulate_contributions()
