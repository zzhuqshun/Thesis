...
class Config:
    def __init__(self, **kwargs):
        ...
        self.BASE_DIR = Path('model/tryouts/minmaxEFC_trial15')
        # Model parameters
        self.SEQUENCE_LENGTH = 726
        self.HIDDEN_SIZE = 32
        self.NUM_LAYERS = 4
        self.DROPOUT = 0.4
        ...
        self.GROUP_BY_CELL = True # Group data by cell_id
        self.FEATURES_COLS = ['Voltage[V]', 'Current[A]', 'Temperature[°C]','EFC']
        self.SEED = 42
        self.RESAMPLE = '10min'
...
class DataProcessor:
    def __init__(self, data_dir, config):
        ...
        self.config = config
        # 给 Voltage/Current/Temperature 用 RobustScaler，给 EFC 用 MinMaxScaler
        self.scaler_num = RobustScaler()
        self.scaler_efc = MinMaxScaler(feature_range=(0, 1))
        self.feature_cols = config.FEATURES_COLS  # e.g. ['Voltage[V]', 'Current[A]', 'Temperature[°C]', 'EFC']
        logger.info("Initialized DataProcessor with data_dir=%s, MODE=%s, features=%s", \
                    self.data_dir, getattr(self.config, 'MODE', None), self.feature_cols)
    ...
    def prepare_data(self):
        ...
        logger.info("Found %d parquet files in %s", len(files), self.data_dir)
        ...
        if self.config.MODE == 'joint':
            ...
            all_train_ids = self.config.dataset_joint.get('train', [])
            all_val_ids = self.config.dataset_joint.get('val', [])
            logger.info("[DATA] Train IDs: %s", all_train_ids)
            logger.info("[DATA] Val IDs: %s", all_val_ids)
            ...
            logger.info("[DATA] joint_train: %d samples", len(datasets['joint_train']))
            dfs_val = [self.process_file(file_map[id]) for id in all_val_ids]
            datasets['joint_val'] = pd.concat(dfs_val, ignore_index=True)
            logger.info("[DATA] joint_val: %d samples", len(datasets['joint_val']))
            # Test full
            test_id = self.config.dataset_joint['test'][0]
            df_test = self.process_file(file_map[test_id])
            datasets['test_full'] = df_test
            logger.info("[DATA] joint_test_full: %d samples", len(df_test))
            ...
            for phase in ['base', 'update1', 'update2']:
                for split in ['train', 'val']:
                    ...
                    summary.append(f"{key}: {len(datasets.get(key, []))} samples, ids={ids}")
            logger.info("[DATA] Split details: %s", "; ".join(summary))
            # Test splits
            ...
            logger.info("[DATA] test_full: %d, base: %d, update1: %d, update2: %d", \
        ...
        # Fit scalers
        # Voltage & Current
        vc_features = ['Voltage[V]', 'Current[A]']
        self.scaler_vc.fit(scaler_data[vc_features])
        logger.info("Fitted RobustScaler on VC features %s", vc_features)
        logger.info("VC centers=%s, scales=%s", self.scaler_vc.center_, self.scaler_vc.scale_)
        # Temperature
        t_feature = ['Temperature[°C]']
        self.scaler_t.fit(scaler_data[t_feature])
        logger.info("Fitted StandardScaler on T feature %s", t_feature)
        logger.info("T mean=%s, var=%s", self.scaler_t.mean_, self.scaler_t.var_)
        # EFC
        self.scaler_efc.fit(scaler_data[['EFC']])
        logger.info("Fitted MinMaxScaler (0,1) on EFC with min=%s, max=%s",
                    self.scaler_efc.data_min_[0], self.scaler_efc.data_max_[0])

        # Transform all splits
        for key, df in datasets.items():
            if df.empty:
                logger.info("Skipping empty dataset split: %s", key)
                continue
            df_scaled = df.copy()
            df_scaled[vc_features] = self.scaler_vc.transform(df[vc_features])
            df_scaled[t_feature]    = self.scaler_t.transform(df[t_feature])
            df_scaled[['EFC']]      = self.scaler_efc.transform(df[['EFC']])
            datasets[key] = df_scaled
            logger.info("Scaled dataset '%s' with shape %s", key, df_scaled.shape)
